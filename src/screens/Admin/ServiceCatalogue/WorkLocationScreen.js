...
const WorkLocationScreen = ({ navigation }) => {
  ...
  const RenderItems = (data) => {
    return (
      <View style={[Styles.backgroundColor, Styles.borderBottom1, Styles.paddingStart16, Styles.flexJustifyCenter, { height: 72 }]}>
        <List.Item title={data.item.workLocationName} titleStyle={{ fontSize: 18 }} description={"Display: " + (data.item.display ? "Yes" : "No")} left={() => <Icon style={{ marginVertical: 12, marginRight: 12 }} size={30} color={theme.colors.textSecondary} name="account-star" />} />
      </View>
    );
  };
  ...
  return (
    <View style={[Styles.flex1]}>
      <Header navigation={navigation} title="WorkLocation" />
      {isLoading ? (
