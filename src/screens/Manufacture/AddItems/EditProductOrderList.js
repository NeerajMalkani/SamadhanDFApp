...
function EditProductOrderList({ route, navigation }) {
  ...
  const FetchServiceNames = () => {
    const params = {
      data: {
        ...
        mf_po_refno: route.params.data.mf_po_refno,
      },
    };
        ...
        .then((response) => {
          if (response.data?.data) {
            setData((prev) => {
              return {
                ...prev,
                coil_details: response.data.data[0].coilsdetails_data,
              };
            });
            setErrors((prev) => {
              return {
  ...
  const update = () => {
    ...
    Provider.createDFManufacturer(
      Provider.API_URLS.manufacturerpoupdate,
      params
    )
      .then((response) => {
...
export default EditProductOrderList;
